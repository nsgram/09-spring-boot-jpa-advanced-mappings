...
public class AppDAOImpl implements AppDAO {
	...
	public Course findCourseAndReviewsByCourseId(int theId) {
		TypedQuery<Course> query = manager.createQuery("select c from Course c JOIN FETCH c.reviews where c.id = :data ", Course.class);
		query.setParameter("data", theId);
		return query.getSingleResult();
	}
	...
	public Course findCourseAndStuedentByCourseId(int theId) {
		TypedQuery<Course> query = manager.createQuery("select c from Course c JOIN FETCH c.students where c.id = :data ", Course.class);
		query.setParameter("data", theId);
		return query.getSingleResult();
	}
	...
	public Student findStudentAndCourseByStudentId(int theId) {
		TypedQuery<Student> query = manager.createQuery("select s from Student s JOIN FETCH s.courses where s.id = :data ", Student.class);
		query.setParameter("data", theId);
		return query.getSingleResult();
	}
	...
	public void deleteStudentById(int theId) {
		Student student = manager.find(Student.class, theId);
